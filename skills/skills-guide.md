...
| 🧠 `brainstorming` | 기능 구현·컴포넌트 작성 전 사용자 의도와 요구사항을 탐색하는 아이디어 발산 스킬 |
| 🎨 `canvas-design` | .png/.pdf 형식의 포스터·아트·디자인 결과물을 디자인 철학 기반으로 생성 |
| 🗺️ `executing-plans` | 작성된 구현 계획을 리뷰 체크포인트와 함께 별도 세션에서 실행 |
...
| 🔍 `receiving-code-review` | 코드 리뷰 피드백 수신 시 맹목적 구현이 아닌 기술적 검증을 거쳐 반영 |
...
| 🐛 `systematic-debugging` | 버그·테스트 실패·비정상 동작 발생 시 수정 전 체계적 원인 분석 |
| 🎨 `tailwind-design-system` | Tailwind CSS v4 기반 확장 가능한 디자인 시스템·컴포넌트 라이브러리 구축 |
| 🧪 `test-driven-development` | 기능 구현·버그 수정 전 테스트를 먼저 작성하는 TDD 워크플로우 |
| 🖼️ `ui-ux-pro-max` | UI/UX 설계 지원 — 161개 컬러 팔레트, 57개 폰트 페어링, 99개 UX 가이드라인 등 |
| 🦸 `using-superpowers` | 대화 시작 시 사용 가능한 스킬을 탐색·활용하는 방법을 확립하는 스킬 |
...
| 📝 `writing-plans` | 스펙·요구사항을 받아 코드 수정 전 다단계 구현 계획서를 작성 |
